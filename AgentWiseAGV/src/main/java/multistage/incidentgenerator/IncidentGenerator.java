  ...
  public IncidentList run() {
    ...
    while (startOfTimeBlock < setting.getEndTime()) {
      if (randomGenerator.nextDouble() < setting.getIncidentRate()) {
        final long incidentStartTime = (long) (startOfTimeBlock + randomGenerator.nextDouble()*setting.getTimeBlock());
        final long incidentDuration = (long) (randomGenerator.nextDouble()*setting.getTimeBlock());
